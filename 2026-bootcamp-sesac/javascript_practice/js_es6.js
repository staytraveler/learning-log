...
const orderLables = orders.map(({ product, price, quantity }) => { return `${product}: ${price * quantity}원` });
...
function printTeam(leader, ...members) {
    ...
    console.log(`팀원:1`, members); //``로 ${}해서 넣으면 배열을 .toString  한 것처럼 돼서 문자열로 나와버림
}
...
const updatedProducts = addedProducts.map(product => product.id === 1 ? { ...product, price: 55000 } : product);

const availableProducts = updatedProducts.filter(product => product.stock > 0);
const availableNames = availableProducts.map(({ name }) => name);

const getTotal = (...prices) => prices.reduce((sum, price) => sum + price, 0);
const prices = updatedProducts.map(products => products.price);
...
console.log('판매 가능 상품:', availableNames);
console.log('전체 상품 가격 합계:', getTotal(...prices));
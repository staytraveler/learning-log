...
// 문제 2 풀이
const notificationCount = 0 ?? 10;
const statusMessage = '' ?? '상태 메세지 없음';
const profileImage = null ?? 'default.png';

console.log(`알림 횟수: ${notificationCount}`);
...
const obj1 = new Product('키보드', 50000, 3);
const obj2 = new Product('마우스', 30000, 5);
...
Member.prototype.introduce = function () {
    const o_name = this.options?.nickname ?? this.name;
    const o_level = this.options?.level ?? '일반';
    return `${o_name}님은 ${o_level} 회원입니다.`;
}
